...
void deleteFunc(Vector<string>& tableColumn, string& val, Schema& schema) {
...
void preDelete(Vector<string>& tokens, Schema& schema) {
    ...
    string pathToLock = schema.name + "/" + tableName + "/" + tableName + "_lock.txt";
    lock(pathToLock);
    deleteFunc(tableColumn, value, schema);
}

void selcet(Vector<string>& tables, Vector<string>& tabColumns, Schema& schema) {
    if (tables.size() == 1) {
...
void selectWhere(Vector<string>& tables, Vector<string>& tabColumns, Vector<string> conditions, Schema& schema) {
    if (tables.size() == 1) {
        string tableName = tables.get(0);
        Vector<string> columns;
        ...
        for (int i = 1; i <= csvCount; i++) {
            ...
            for (int lineIndex = 1; lineIndex < csvData.size(); lineIndex++) { // всё кроме заголовка
                ...
                if (isOrCondition) {
                    for (int i = 0; i < indexCol.size(); i++) {
                        cout << lineElements.get(indexCol.get(i));
...
void preSelect(Vector<string>& tokens, Schema& schema) {
    ...
    if (!isWhere) {
        selcet(tables, tabColumns, schema);
    } else {
        selectWhere(tables, tabColumns, conditions, schema);
    }
}
